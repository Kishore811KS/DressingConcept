...
import calendar
from app import db
from app.models import Salary, Employee, Attendance, HRConfig, AdvanceSalary, Bill, SaleReturn
...
def calculate_salaries():
    ...
    try:
        ...
        for emp in employees:
            ...
            # Calculate total sales generated by employee in the month
            emp_name = emp.full_name or ''
            bills_in_month = Bill.query.filter(
                func.month(Bill.created_at) == month,
                func.year(Bill.created_at) == year,
                Bill.created_by_name.ilike(f"%{emp_name}%")
            ).all() if emp_name else []

            returns_in_month = SaleReturn.query.filter(
                func.month(SaleReturn.created_at) == month,
                func.year(SaleReturn.created_at) == year,
                SaleReturn.created_by_name.ilike(f"%{emp_name}%")
            ).all() if emp_name else []

            total_sales_val = sum(b.total or 0 for b in bills_in_month)
            total_returns_val = sum(r.total_return_amount or 0 for r in returns_in_month)
            total_sales_generated = max(0.0, total_sales_val - total_returns_val)
            ...
            res_dict.update({
                ...
                'total_sales_generated': round(total_sales_generated, 2)
            })
...
@salary_bp.route('/update-incentive', methods=['PUT'])
def update_salary_incentive():
    """Update incentive amount manually for an employee's salary record"""
    _ensure_salary_columns()
    try:
        data = request.get_json()
        salary_id = data.get('salary_id')
        ...
        year = data.get('year')
        incentive_amount = float(data.get('incentive_amount', 0.0) or 0.0)
        
        salary_record = None
        if salary_id:
            salary_record = Salary.query.get(salary_id)
        if not salary_record and employee_id and month and year:
            salary_record = Salary.query.filter_by(employee_id=employee_id, month=month, year=year).first()
        ...
        if not salary_record and employee_id and month and year:
            ...
            salary_record.incentive_amount = max(0.0, incentive_amount)
        else:
            return jsonify({'error': 'Salary record not found'}), 404
